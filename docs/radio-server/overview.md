...
2.  **Process 2: Converter Task**: Receives the raw PCM periods, encodes them directly into multiple qualities (e.g., HQ FLAC and downsampled LQ FLAC). It assembles these into complete 10-second segments and sends the completed segments to the Cloud Uploader Task via a bounded `mpsc` channel (capacity 3).
