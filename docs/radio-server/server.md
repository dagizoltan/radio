...
**Segment boundary:** When the nominal 480,000-frame threshold is reached and a segment is finalized, **do not flush** the `opus_staging` buffer. The `libopus` encoder strictly requires input sizes to exactly match the configured frame size (960 frames). Passing a smaller, arbitrary remainder (e.g., 400 frames) will result in an `OPUS_BAD_ARG` error. Instead, because the LQ stream is a continuous gapless format without Ogg boundaries, simply leave any remaining PCM samples in `opus_staging`. They will be carried over and prepended to the next ALSA period received in the following segment. The continuous encoder state must **not** be reset.
...
4.  **Segment Assembly:** Accumulates the encoded frames for both qualities.
    *   *Optimization Strategy:* To prevent constant vector reallocations as frames accumulate, the accumulator `Vec<u8>` for the 10-second segment must be pre-allocated with `Vec::with_capacity()`. A nominal 10-second 24-bit verbatim FLAC segment contains the raw PCM payload (around `48000 * 3 bytes * 2 channels * 10s = 2,880,000` bytes) plus the FLAC framing overhead. Because the ALSA period is 4096 frames and 480,000 is not perfectly divisible by 4096, segments contain either 117 periods (479,232 frames) or 118 periods (483,328 frames). An upper bound 118-period segment contains `483,328 * 6 = 2,899,968` bytes of PCM. With 118 FLAC frames each contributing a header (~10-15 bytes) and CRC-16 (2 bytes), the overhead is around ~2,000 bytes. To ensure zero reallocations, the buffer must be padded safely above the largest possible segment size.
    *   **Threshold:** 480,000 frames (10 × 48000 Hz) nominal target. Track a `frame_counter: u64` incremented by `period_frames` (4096) per ALSA period. Pre-allocate accumulator: `Vec::with_capacity(2_905_000)` for HQ (max PCM + framing overhead padding).
