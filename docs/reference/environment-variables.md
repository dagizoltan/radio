...
| `R2_PUBLIC_URL` | `radio-client` | (None) | The public-facing base URL for fetching the manifest and segments. | Read by `Deno.env.get("R2_PUBLIC_URL")` in `main.tsx` for all proxy fetch requests. |
