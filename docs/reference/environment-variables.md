...
| `R2_BUCKET` | `radio-server`, `client` (via Compose) | `radio-stream` | The name of the S3 bucket to upload to/proxy from. | Read by the S3 Uploader task for path construction; read by docker-compose to construct `R2_PUBLIC_URL`. |
...
| `R2_PUBLIC_URL` | `radio-client` | (None) | The public-facing base URL for fetching the manifest and segments. | Read by `Deno.env.get("R2_PUBLIC_URL")` in `main.tsx` to proxy the manifest and inject the base URL into the Web Component's direct segment fetch logic. |
