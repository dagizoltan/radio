...
| `PORT` | `radio-client` | `3000` | The HTTP port the Hono server binds to. | Read by `Deno.env.get("PORT")` in `main.js`. |
...
| `TOKEN_SECRET` | `radio-client` | (None, optional) | HMAC secret for generating short-lived segment access tokens injected into the HTML. If unset, token injection is disabled. | Read by `Deno.env.get("TOKEN_SECRET")` in `main.js`. |