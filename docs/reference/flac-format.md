...
2.  **STREAMINFO Block (34 bytes):**
    ...
    *   Bits per sample: `5` bits (`15` for 16-bit)
...
7.  **Sample Size Code (3 bits):** Always `0b100` (16 bits per sample).
...
2.  **Uncompressed Audio Data:** `4096` consecutive 16-bit samples for that channel, stored as big-endian (or written byte-aligned). Since bits-per-sample is 16, this aligns perfectly to byte boundaries.
