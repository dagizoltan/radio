...
## 3. Testing Contract
- **WASM Bounds Checking:** Write a Deno unit test (`deno test`) that initializes the WASM module and feeds it a truncated FLAC file (cut off mid-frame). Assert that `wasm.push()` does not panic, and successfully returns the `f32` floats parsed *up to* the truncation point, leaving the remainder in the accumulator.
- **Normalization Verification:** In the Deno test, push a synthetic FLAC frame containing the maximum positive 24-bit value (`0x7FFFFF`). Assert the resulting `f32` output is exactly `0.99999988` (i.e. `8388607 / 8388608`).

## 4. Error Recovery Matrix
- **Corrupt R2 Manifest:** If the Deno server fetches `manifest.json` from R2 and receives a `502 Bad Gateway` or invalid JSON, it must NOT crash the SSR render. It must catch the error and render the HTML shell with `data-live="false"` and fallback defaults, allowing the client to retry later.
- **WASM Memory Detachment:** If JS attempts to clone the `pcmView.buffer` (which points to WASM linear memory), the WASM module will immediately detach and crash on the next `push()`. The code must strictly use `Float32Array.set()` to copy values out of the WASM heap.
