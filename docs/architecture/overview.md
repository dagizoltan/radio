...
6. **Listener Client:** The listener visits the Deno Deploy frontend (`radio-client`). The frontend fetches the manifest and serves the HTML shell and static assets (JS, WASM).
7. **Browser Playback:** The browser loads a Web Component island that continuously polls the manifest and fetches audio segments *directly* from the Cloudflare R2 edge CDN (bypassing the Deno proxy to save bandwidth). The segments are decoded in the browser using a [WASM FLAC decoder](../radio-client/wasm-decoder.md) and played via an [AudioWorklet](../radio-client/worklet.md).
