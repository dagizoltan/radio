...
12. **Browser Fetch & Decode:** The `radio-player` Web Component in the browser fetches the segment chunk-by-chunk using a `ReadableStream`. Each chunk is passed to the [WASM Decoder](../radio-client/wasm-decoder.md), which parses either the FLAC or MP3 stream (depending on user selection) and yields `f32` PCM data.
