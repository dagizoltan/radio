...
1.  **Accumulation:** The Converter Task accumulates encoded frames until 491,520 frames of PCM audio have been processed (exactly 10.24 seconds × 48000 Hz = 491,520 frames). Frame count, not byte count, is used as the threshold to remain independent of sample packing format (S24_LE, S32_LE, etc.). Because 491,520 is perfectly divisible by both the ALSA period size (4096 frames) and the Opus frame size (960 frames), exactly 120 ALSA periods form a perfectly aligned 10.24-second segment without any leftovers. The Uploader Task receives complete pre-assembled segments — it does not accumulate frames itself.
