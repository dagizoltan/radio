...
1.  **Accumulation:** The Converter Task accumulates encoded frames until at least 480,000 frames of PCM audio have been processed (10 seconds × 48000 Hz = 480,000 frames). Frame count, not byte count, is used as the threshold to remain independent of sample packing format (S24_LE, S32_LE, etc.). The Uploader Task receives complete pre-assembled segments — it does not accumulate frames itself.
    *   *Frame Alignment Note:* ALSA provides periods of 4096 frames. Because 480,000 is not perfectly divisible by 4096 (it equals ~117.18 periods), segments will contain either 117 periods (479,232 frames, ~9.984s) or 118 periods (483,328 frames, ~10.069s) depending on exactly when the threshold is crossed and how the state is maintained. 10 seconds is a nominal target, not an exact guarantee per segment.
