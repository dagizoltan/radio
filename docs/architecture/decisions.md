...
The Converter process encodes the lower-quality (LQ) fallback stream as Opus at 128 kbps stereo using the pure-Rust `audiopus` crate for encoding. It uses a custom length-prefixed binary format to package the raw Opus packets instead of an Ogg container.
...
*   **Rationale:** Cloudflare R2 does not charge for egress bandwidth, but it *does* charge for Class B operations (GET requests). If thousands of clients poll `manifest.json` directly from the R2 bucket every 10 seconds, the Class B operations bill will skyrocket (e.g., ~$460/month for 5,000 users).
*   **Constraint:** The `manifest.json` must **not** be exposed directly to clients with `no-store`. Instead, the Deno frontend must proxy the manifest fetch and return it with a `Cache-Control: s-maxage=5` header (or rely on a Cloudflare Cache Rule). This coalesces thousands of client polls into a single bucket read every few seconds, dropping Class B costs to near zero.
