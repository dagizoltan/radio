...
*   **Rationale:** Cloudflare R2 does not have a native, immediate Time-to-Live (TTL) feature that deletes objects precisely when they expire. To prevent unbounded storage growth for a continuous live stream, the server must manage the lifecycle manually. A rolling window ensures exactly 10 segments are stored per quality stream at any time. A 10-segment window (~100 seconds of audio) provides a wide buffer against transient 404 errors caused by manifest fetch races, slight clock drift between clients, or jump-ahead buffering logic, while still maintaining minimal storage footprint.
...
*   **Constraint:** Rolling window, not TTL. The uploader maintains a `VecDeque` of uploaded keys and deletes the oldest immediately when the window exceeds 10 segments, while relying on the cloud platform's bucket policy for an ultimate backup.
...
- **Pre-skip gap:** The Opus spec requires each new encoder stream to output `pre_skip` samples of encoder lookahead before the first audio packet. At 48000 Hz with the default `pre_skip` of 312 samples (~6.5ms), each 10-second `.opus` segment introduces a ~6.5ms gap of silence at its start. The `OpusDecoder` discards these pre-skip samples as documented in `wasm-decoder.md`. Over 360 segments per hour, the cumulative gap in the LQ stream is approximately 2.3 seconds per hour of broadcast. This is inherent to segmented Opus streaming and is an accepted trade-off for the simplicity of self-contained segment files. Because each decoded LQ segment yields approximately 312 fewer PCM frames than the nominal 480,000, the latency display (`(latest - currentIndex) * segment_s`) will slightly underestimate the true live edge gap over long sessions — accumulating roughly 2.3 seconds of error per broadcast hour. The fetch loop itself is unaffected: it advances to the next segment index as soon as the current segment's `ReadableStream` is exhausted, regardless of decoded frame count. The HQ FLAC stream has no equivalent gap.
- **Padding elimination:** At the end of a segment, if the remaining samples do not fit precisely into a 960-frame boundary, the system must **not** pad with silence. Doing so injects an audible gap every 10 seconds on the LQ stream. The `audiopus` encoder must flush the exact number of remaining frames, adjusting the Ogg granule position accordingly, ensuring the decoder produces a gapless audio stream (barring the `pre_skip` mentioned above).
...
*   **Rationale:** Hardware clocks are never perfectly synchronized. If the server ADC captures at 48001 Hz and the client DAC plays at 47999 Hz, the client's internal ring buffer will slowly bloat by 2 frames per second (150ms over an hour), causing latency to creep upwards. Conversely, if the client DAC plays faster, it will eventually drain the buffer and stall.
*   **Implementation:** The client continuously monitors its internal buffer depth. While explicit threshold events (like tab visibility changes) handle large jumps via an immediate queue flush, continuous micro-drift is managed naturally by the `AudioWorklet`. When the worklet reads samples, if the buffer exceeds a safe upper bound (e.g., 2.5 segments), it can implicitly drop a tiny, inaudible fraction of frames, or the fetch loop's jump-ahead logic (`currentIndex < latest - 3`) will naturally act as a hard cap on buffer bloat.
