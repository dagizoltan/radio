    ...
    pub async fn read_period(&self) -> std::io::Result<(Vec<i32>, bool)> {
        loop {
            ...
            if ret < 0 {
                ...
                let r_offset = if samples_per_frame > 1 { 1 } else { 0 };
                ...
                if self.format == SNDRV_PCM_FORMAT_S24_3LE {
                    ...
                    for frame in 0..frames_read {
                        ...
                        let r_base = base + (r_offset * 3);
                        let r_bytes = [raw_bytes[r_base], raw_bytes[r_base + 1], raw_bytes[r_base + 2]];
                    ...
                    for frame in 0..frames_read {
                        ...
                        let r_raw = raw_i16[base + r_offset] as i32;
                    ...
                    for frame in 0..frames_read {
                        ...
                        let r_raw = raw_buffer[base + r_offset];
